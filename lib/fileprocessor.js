...
var _defaultPatterns = {
	'html': [
      ...
      /*jshint regexp:false */
      [ /<script.+src=["'](?:@Url.Content\(")([^'"]+)["']/gm,
        'Update the ASP.NET MVC HTML to reference our concat/min/revved script files',
         null,
         fixMVCPath
      ],
      [ /<link[^\>]+href=['"]([^"']+)["']/gm,
      ...
      [ /<link[^\>]+href=['"](?:@Url.Content\(")([^"']+)["']/gm,
        'Update the ASP.NET MVC HTML with the new css filenames',
        null,
        fixMVCPath
      ],
      [ /<img[^\>]+src=['"]([^"']+)["']/gm,
...
FileProcessor.prototype.replaceWith = function replaceWith(block) {
  ...
  if (block.isAspmvc === true) {
    block.dest = block.dest.replace(/dist[\\/]~[\\/]dist/, '~/dist');
  }
  ...
  if (typeof block.src === 'undefined' || block.src === null || block.src.length === 0) {
    ...
    result = block.indent + conditionalStart + '<link rel="stylesheet" href="' + (block.isAspmvc ? '@Url.Content("' : '') + dest  + (block.isAspmvc ? '")' : '') + '"' + media + '/>' + conditionalEnd;
  } else if (block.defer) {
    result = block.indent + conditionalStart + '<script defer src="' + (block.isAspmvc ? '@Url.Content("' : '') + dest  + (block.isAspmvc ? '")' : '') + '"><\/script>' + conditionalEnd;
  } else if (block.async) {
    result = block.indent + conditionalStart + '<script async src="' + (block.isAspmvc ? '@Url.Content("' : '') + dest  + (block.isAspmvc ? '")' : '') + '"><\/script>' + conditionalEnd;
  } else if (block.type === 'js') {
    result = block.indent + conditionalStart + '<script src="' + (block.isAspmvc ? '@Url.Content("' : '') + dest  + (block.isAspmvc ? '")' : '') + '"><\/script>' + conditionalEnd;
  } /*else {
    result = '';
  }*/
  return result;
...
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath) {
    ...
    regexps.forEach(function (rxl) {
      ...
      content = content.replace(rxl[0], function (match, src) {
        ...
        var resfile = filterOut(file);
        var res = match.replace(src, resfile);
        if (resfile !== src) {
          self.log(srcfile + ' changed to ' + resfile);
        }
        return res;
      });
    });
